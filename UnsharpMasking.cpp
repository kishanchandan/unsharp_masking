...
void CKingimageView::OnUnsharpmasking()
{
	...
	if (iBitPerPixel == 8)  ////Grey scale 8 bits image
	{
		...
		for (int i = 1; i < iHeight - 1; i++)
			...
			{
				blur[i*Wp + j] = (pImg[(i - 1)*Wp + (j - 1)] * mask[0] + pImg[(i - 1)*Wp + j] * mask[1] + pImg[(i - 1)*Wp + (j + 1)] * mask[2] + pImg[i*Wp + (j - 1)] * mask[3] + pImg[i*Wp + j] * mask[4] + pImg[i*Wp + (j + 1)] * mask[5] + pImg[(i + 1)*Wp + (j - 1)] * mask[6] + pImg[(i + 1)*Wp + j] * mask[7] + pImg[(i + 1)*Wp + (j + 1)] * mask[8]) / 9;
				unmask[i*Wp + j] = pImg[i*Wp + j] - blur[i*Wp + j];
				pImg[i*Wp + j] += unmask[i*Wp + j];
				pImg[i*Wp + j] = ((pImg[i*Wp + j] > 255) ? 255 : pImg[i*Wp + j]);
				pImg[i*Wp + j] = ((pImg[i*Wp + j] <0) ? 0 : pImg[i*Wp + j]);
			}
	}
	OnDraw(GetDC());
}
